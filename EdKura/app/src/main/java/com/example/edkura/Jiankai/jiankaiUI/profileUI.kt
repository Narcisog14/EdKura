...
class CustomCanvasView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private val maxRectWidthFactor = 0.7f  // 最大宽度 70%
    ...
    private var overlayAlpha = 0 // 遮罩层透明度
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (isDraggingRect) {
                    // 计算滑动的距离
                    val swipeDistance = Math.abs(event.x - initialTouchX)

                    // 如果滑动距离大于一定阈值且矩形已展开，尝试关闭矩形（向右滑动）
                    if (swipeDistance > 100 && isRectOpen) {  // 100是滑动的最小距离阈值
                        animateRectangle(rectWidthFactor, 0.05f, overlayAlpha, 0)
                        isRectOpen = false
                    } else {
                        // 如果矩形处于打开状态，判断是否完全展开
                        if (rectWidthFactor > (maxRectWidthFactor / 4)) {
    ...
    private inner class GestureListener : GestureDetector.SimpleOnGestureListener() {
        override fun onFling(
            ...
            if (e1 != null && !isDraggingRect) {
                ...
                // 向右快速滑动打开矩形
                if (!isRectOpen && distanceX > 50 && Math.abs(velocityX) > 100) {
                    isExpandingRect = true // 标记为正在展开
                    animateRectangle(rectWidthFactor, maxRectWidthFactor, overlayAlpha, 100)
                    isCurrentlyScrolling = true  // 标记为滑动状态
    ...
    private fun drawFixedPath(canvas: Canvas, rectWidth: Float) {
        ...
        val path = Path().apply {
            moveTo(rectWidth-1f, squareHeight - cornerRadius)
            ...
            quadTo(rectWidth-1f, squareHeight + squareWidth, rectWidth, squareHeight + squareWidth + cornerRadius)
        }
