...
class CustomCanvasView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
        ...
        if (user != null) {
            ...
            FirebaseDatabase.getInstance().reference
                ...
                .addOnSuccessListener { snapshot ->
                    userName = snapshot.getValue(String::class.java) ?: userEmail
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        drawTextInRect(canvas, "Email: $userEmail", rectWidth, height-200f)
    }

    private fun drawTextInRect(canvas: Canvas, text: String, rectWidth: Float, rectHeight: Float) {
        val textPaint = Paint().apply {
            color = Color.BLACK  // 文字颜色
            textSize = 40f  // 文字大小
            textAlign = Paint.Align.RIGHT  // 文字靠右对齐
        }
        ...
        // 计算文字的 x 和 y 坐标
        val textX = rectWidth - 50f  // 文字的右边紧贴 rectWidth，留一点边距
        val textY = (rectHeight + textHeight) / 2 - textPaint.fontMetrics.descent  // 垂直居中
...
// 加载图案
        val drawable = ContextCompat.getDrawable(context, R.drawable.jk_icon_settings)
