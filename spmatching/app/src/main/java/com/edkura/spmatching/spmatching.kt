...
import com.edkura.spmatching.R.layout.br_spmatching
    ...
    private fun sendStudyPartnerRequest(receiverId: String) {
        ...
        query.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (duplicateFound) {
                    Toast.makeText(this@spmatching, "You already sent a pending request to $receiverId", Toast.LENGTH_SHORT).show()
                } else {
                    ...
                    newRequestRef.setValue(requestData)
                        .addOnSuccessListener {
                            Toast.makeText(this@spmatching, "Request sent to $receiverId", Toast.LENGTH_SHORT).show()
                        }
                        .addOnFailureListener { e ->
                            Toast.makeText(this@spmatching, "Failed to send request: ${e.message}", Toast.LENGTH_SHORT).show()
                        }
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@spmatching, "Error checking existing requests: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
